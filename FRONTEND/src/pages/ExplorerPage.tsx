      ...
      <div className="explorer-layout">
        ...
        <main className="explorer-main">
          <div className="search-box-premium" style={{ margin: '0 0 30px 0', maxWidth: 'none' }}>
            <input type="text" placeholder="🔍 ¿Qué quieres aprender hoy?..." value={searchTerm} onChange={(e) => {setSearchTerm(e.target.value); setCurrentPage(1);}} />
            <button className="btn-primary-levelup" onClick={onSearch}>Buscar</button>
          </div>

          <div className="jobs-list" style={{ justifyContent: 'flex-start' }}>
            {currentCourses.length > 0 ? (
              currentCourses.map((c: any) => (
                <CourseCard key={c.id} course={c} isFavorite={favorites.includes(c.id)} onFavorite={toggleFavorite} onClick={() => { setSelectedCourse(c); navigateTo('course-detail') }} />
              ))
            ) : (
          ...
          {/* NUMERITOS DE PAGINACIÓN */}
          {totalPages > 1 && (
            <div className="pagination">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(num => (
                <button key={num} onClick={() => { setCurrentPage(num); window.scrollTo(0,0); }} className={`page-btn ${currentPage === num ? 'active' : ''}`}>
                  {num}
                </button>
              ))}
            </div>
