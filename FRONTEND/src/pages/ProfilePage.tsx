...
const ProfilePage: React.FC<ProfilePageProps> = ({ token, handleLogout }) => {
  ...
  const fetchProfile = () => {
    if (token) {
      axios.get('https://levelupacademyterminado.onrender.com/api/profile', {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.patch('https://levelupacademyterminado.onrender.com/api/profile', 
        { user: cleanData }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDeleteAccount = async () => {
    ...
    if (confirm && token) {
      try {
        await axios.delete('https://levelupacademyterminado.onrender.com/api/profile', {
          headers: { Authorization: `Bearer ${token}` }
        });
